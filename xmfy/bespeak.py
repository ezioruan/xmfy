...
def despeak(doctor_id, select_date, select_time, phone, icardid, username, dept_name=DEPT_NAME_BOBY):
    doctor = ALL_DOCTORS.get(doctor_id)
    param_dict = {'DOCTOR_ID':doctor_id, 'DOCTOR':doctor, 'SelectDate':select_date,
                  'SelectTime':select_time, 'DEPT_NAME':dept_name, 'phone':phone,
                  'icardid':icardid, 'username':username}
    return post_url(XMFY_DESPEAK_FORM_URL, param_dict)
    


def test_despeak():
#     print despeak('1263','2013-4-26','11：30','13625002107','350681198812051039',u'阮明辉',DEPT_NAME_WOMAM)
    ...
    for time in time_list:
        soup = despeak('092', '2013-4-27', time, '15859251674', 'D75698209', u'石锦芬', DEPT_NAME_BOBY)
        print '[%s]:%s' % (time,soup.find('script'))
